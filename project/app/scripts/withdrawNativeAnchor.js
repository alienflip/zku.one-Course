...
async function generateSnarkProof(deposit, recipient, contractAddress) {
  // find the inputs that correspond to the path for the deposit
  const { root, path_elements, path_index } = await generateMerkleProof(deposit, contractAddress);
...
async function runScript(noteString, recipientAddress, contractAddress) {
  return await withdraw(noteString, recipientAddress, contractAddress);
}
